...
{
    int[,] result = new int[matrix.GetLength(1), matrix.GetLength(0)]; // меняем местами строки на столбцы и столбцы на строки у новой матрицы. Тогда можем вывести не только квадратную матрицу, но меняется размерность матрицы
    for (int i = 0; i < matrix.GetLength(0); i++)
    {
        for (int j = 0; j < matrix.GetLength(1); j++)
        {
...
void MatrixTransposition2(int[,] matrix) // работает только для квадратных матриц
{
